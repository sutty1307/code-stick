#!/usr/bin/env bash
# code-stick + OpenClaw Chat Interface (macOS / Linux)
...
if ! curl -s http://127.0.0.1:11434/api/tags &>/dev/null; then
    ...
    OLLAMA_BIN=""
    for candidate in \
        "$USB_ROOT/bin/ollama-darwin-arm64" \
        "$USB_ROOT/bin/ollama-darwin-x64" \
        "$USB_ROOT/bin/ollama" \
...
echo "==========================================================="
echo ""
echo "Press ENTER to stop OpenClaw (Ollama keeps running)..."
read -r
